...
// NOTE: If this starts slowing down, implement partition pruning on recent dates.
val dfMain = sqlContext.table("cis.pros_bid_price_hist")
...
val files = fs.globStatus(new Path("pros/asiamiles_bidprice_*.csv*"))
...
val csvSchema = StructType(Array(
    ...
    StructField("seat_idx", IntegerType, false),
    StructField("bid_value", IntegerType, false)))
...
val dfStage = dfCsv
    ...
    .select("seg_dep_date", "carrier_code", "flt_num", "orig_apt", "dest_apt", "compt", "seat_idx", "bid_value", "snapshot_date", "snapshot_year", "snapshot_month")

dfStage.write.partitionBy("snapshot_year", "snapshot_month").insertInto("cis.pros_bid_price_hist")
